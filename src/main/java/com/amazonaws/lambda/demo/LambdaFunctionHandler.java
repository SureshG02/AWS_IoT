...
public class LambdaFunctionHandler implements RequestHandler<Object, String> {
	...
	public String handleRequest(Object input, Context context) {
		...
		final String SERAIL_NUMBER = System.getenv("SERAIL_NUMBER");
		...
		if (inputJson.contains("Scheduled Event")) {
			...
			try {
				...
				if (group.getDesiredCapacity() > 0) {
					// Assemble the email.
					String BODY = "There are running servers. If they are not in use please press AWS IoT button to shutdown.";
					...
					String BODY = "There are no servers running. Please press AWS IoT button to start all servers.";
					Content textBody = new Content().withData(BODY);
					Body body = new Body().withText(textBody);
					// Create a message with the specified subject and body.
					Message message = new Message().withSubject(subject).withBody(body);
		...
		// Starts and stops all instances after IOT button is pressed.
		if (inputJson.contains(SERAIL_NUMBER)) {
			if (group.getDesiredCapacity() > 0) {
